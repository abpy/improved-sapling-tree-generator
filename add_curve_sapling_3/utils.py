...
def growSpline(n,stem,numSplit,splitAng,splitAngV,splineList,attractUp,hType,splineToBone):
    ...
    if numSplit > 0:
        ...
        for i in range(numSplit):
            ...
            if stem.seg != stem.segMax:
                ...
                splineList.append(stemSpline(newSpline,stem.curv,stem.curvV,stem.seg+1,stem.segMax,stem.segL,stem.children,stem.radS,stem.radE,len(cu.splines)-1))
        ...
        for k in range(curveRes[n]):
            ...
            for spl in tempList:
                ...
                if (k == int(curveRes[n] / 2 + 0.5)) and (curveBack[n] != 0):
                    ...
                    spl.curvAdd(new)
                growSpline(n, spl, numSplit, splitAngle[n], splitAngleV[n], splineList, vertAtt, handles, splineToBone)  # Add proper refs for radius and attractUp
